...
fn p_byte_size<'src>() -> impl Parser<'src, &'src str, i64, OurErr<'src>> + Clone {
    let p_suffix = p_int()
        .then(
            choice((
                just("KiB").to(1024),
                just("MiB").to(1024 * 1024),
                just("GiB").to(1024 * 1024 * 1024),
                just("KB").to(1000),
                just("MB").to(1000 * 1000),
                just("GB").to(1000 * 1000 * 1000),
            ))
            .labelled("file size"),
        )
        .map(|(it, factor)| it * factor);
...
fn p_atom<'src>() -> impl Parser<'src, &'src str, Expr<Raw>, OurErr<'src>> + Clone {
    p_time_span()
        ...
        .or(p_byte_size().map(|it| Expr::Number(it, Raw)))
...
mod tests {
    ...
    fn byte_size_parts_strategy() -> impl Strategy<Value = Vec<(i64, &'static str)>> {
        prop::collection::vec(
            prop_oneof![
                (0_i64..=4096).prop_map(|value| (value, "KiB")),
                (0_i64..=1024).prop_map(|value| (value, "MiB")),
                (0_i64..=256).prop_map(|value| (value, "GiB")),
                (0_i64..=4096).prop_map(|value| (value, "KB")),
                (0_i64..=1024).prop_map(|value| (value, "MB")),
                (0_i64..=256).prop_map(|value| (value, "GB")),
            ],
            1..=4,
        )
    }

    fn render_byte_size(parts: &[(i64, &'static str)]) -> String {
        parts
            .iter()
            .map(|(value, unit)| format!("{value}{unit}"))
            .collect::<String>()
    ...
    fn byte_size_to_bytes(parts: &[(i64, &'static str)]) -> i64 {
        parts
            .iter()
            .map(|(value, unit)| {
                value
                    * match *unit {
                        ...
                        "MB" => 1000 * 1000,
                        "GB" => 1000 * 1000 * 1000,
                        _ => unreachable!("byte size strategy only emits known units"),
                    }
            })
            .sum()
    }

    fn byte_size_literal_strategy() -> impl Strategy<Value = String> {
        byte_size_parts_strategy().prop_map(|parts| render_byte_size(&parts))
    }
    ...
    proptest! {
        ...
        #[test]
        fn parse_expr_accepts_generated_byte_sizes(size in byte_size_literal_strategy()) {
            prop_assert!(
                parse_expr(&size).is_ok(),
                "generated byte size failed to parse: {size}"
            );
        }
        ...
        #[test]
        fn parse_expr_byte_sizes_convert_to_bytes(parts in byte_size_parts_strategy()) {
            let span = render_byte_size(&parts);
            let expected = byte_size_to_bytes(&parts);
            let parsed = parse_pretty(&span);
